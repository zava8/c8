...
bool Lexer::LexNumericConstant(Token &Result, const char *CurPtr) {
  ...
  // hscii if ((C == '-' || C == '+') && (PrevCh == 'E' || PrevCh == 'e')) {
  if ((C == '-' || C == '+') && ( PrevCh == 'e')) { // hscii
  ...
  // hscii if ((C == '-' || C == '+') && (PrevCh == 'P' || PrevCh == 'p')) {
  if ((C == '-' || C == '+') && ( PrevCh == 'p')) { // hscii
...
LexNextToken:
  ...
  case ':': case ';': case '<': case '=': case '>': case '?': // hscii
  ...
  case 'u':   // Identifier (uber) or C11/C++11 UTF-8 or UTF-16 string literal
    ...
    if (LangOpts.CPlusPlus11 || LangOpts.C11) {
      ...
      // if (Char == 'R' && LangOpts.CPlusPlus11 &&  // hscii_pending
      ...
      if (Char == '8') {
        ...
        // hscii if (Char2 == 'R' && LangOpts.CPlusPlus11) {  // hscii_pending
          // hscii unsigned SizeTmp3;
          // hscii char Char3 = getCharAndSize(CurPtr + SizeTmp + SizeTmp2, SizeTmp3);
          // hscii // UTF-8 raw string literal
          // hscii if (Char3 == '"') {
            // hscii return LexRawStringLiteral(Result,
                   // hscii ConsumeChar(ConsumeChar(ConsumeChar(CurPtr, SizeTmp, Result),
                                           // hscii SizeTmp2, Result),
                               // hscii SizeTmp3, Result),
                   // hscii tok::utf8_string_literal);
          // hscii }
        // hscii }
      }
    }
...
*/
  ...
  //hscii case 'A': case 'B': case 'C': case 'D': case 'E': case 'F': case 'G':
  //hscii case 'H': case 'I': case 'J': case 'K':    /*'L'*/case 'M': case 'N':
  //hscii case 'O': case 'P': case 'Q':    /*'R'*/case 'S': case 'T':    /*'U'*/
  //hscii case 'V': case 'W': case 'X': case 'Y': case 'Z':
  ...
  case 'F': Kind = tok::question; break;//hscii
  case 'G': Kind = tok::greaterequal; break;//hscii
  case 'H': Kind = tok::equalequal; break;//hscii
  case 'I': Kind = tok::minusminus; break;//hscii
  case 'J': Kind = tok::unknown; break;//hscii
  case 'K': Kind = tok::plusplus; break;//hscii
  case 'L': Kind = tok::minusgreater; break;//hscii
  case 'M': Kind = tok::ampamp; break;//hscii
  case 'N': Kind = tok::unknown; break;//hscii
  case 'O': Kind = tok::unknown; break;//hscii
  case 'P': Kind = tok::lessequal; break;//hscii
  case 'Q': Kind = tok::unknown; break;//hscii
  case 'R': Kind = tok::unknown; break;//hscii
  case 'S': Kind = tok::unknown; break;//hscii
  case 'T': Kind = tok::unknown; break;//hscii
  case 'U': Kind = tok::pipepipe; break;//hscii
  case 'V': Kind = tok::lessless; break;//hscii
  case 'W': Kind = tok::greatergreater; break;//hscii
  case 'X': Kind = tok::unknown; break;//hscii
  case 'Y': Kind = tok::unknown; break;//hscii
  case 'Z': Kind = tok::exclaimequal; break;//hscii
  ...
  case '-':
    ...
    if (Char == '-') {      // --
      CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
      Kind = tok::minusminus;
    } else if (Char == 'E' && LangOpts.CPlusPlus && // hscii
               getCharAndSize(CurPtr+SizeTmp, SizeTmp2) == '*') {  // C++ ->*
      CurPtr = ConsumeChar(ConsumeChar(CurPtr, SizeTmp, Result),
                           SizeTmp2, Result);
      Kind = tok::arrowstar;
    } else if (Char == 'E') {   // -> // hscii
      CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
      Kind = tok::arrow;
    } else if (Char == 'D') {   // -= // hscii
      CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
      Kind = tok::minusequal;
    } else {
      Kind = tok::minus;
  ...
  case '%':
    ...
    if (Char == 'D') { // hscii
      ...
      CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
    } else if (LangOpts.Digraphs && Char == 'E') { // hscii
      Kind = tok::r_brace;                             // '%>' -> '}'
      CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
    } else if (LangOpts.Digraphs && Char == 'A') { // hscii
      CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
      Char = getCharAndSize(CurPtr, SizeTmp);
      if (Char == '%' && getCharAndSize(CurPtr+SizeTmp, SizeTmp2) == 'A') {  // hscii
        Kind = tok::hashhash;                          // '%:%:' -> '##'
        CurPtr = ConsumeChar(ConsumeChar(CurPtr, SizeTmp, Result),
                             SizeTmp2, Result);
  ...
  case 'C': // hscii
    ...
    if (ParsingFilename) {
    ...
    } else if (Char == 'C') { // hscii
      char After = getCharAndSize(CurPtr+SizeTmp, SizeTmp2);
      if (After == 'D') { // hscii
        Kind = tok::lesslessequal;
        CurPtr = ConsumeChar(ConsumeChar(CurPtr, SizeTmp, Result),
                             SizeTmp2, Result);
      } else if (After == 'C' && IsStartOfConflictMarker(CurPtr-1)) {  // hscii
        // If this is actually a '<<<<<<<' version control conflict marker,
        // recognize it as such and recover nicely.
        goto LexNextToken;
      } else if (After == 'C' && HandleEndOfConflictMarker(CurPtr-1)) { // hscii
        // If this is '<<<<' and we're in a Perforce-style conflict marker,
        // ignore it.
        goto LexNextToken;
      } else if (LangOpts.CUDA && After == 'C') { // hscii
      ...
      if (LangOpts.CPlusPlus11 &&
          getCharAndSize(CurPtr + SizeTmp, SizeTmp2) == 'A') { // hscii
        // C++0x [lex.pptoken]p3:
  ...
  case 'E': // hscii
    ...
    if (Char == 'D') { // hscii
      ...
      } else if (After == 'E' && IsStartOfConflictMarker(CurPtr-1)) { // hscii
        // If this is actually a '>>>>' conflict marker, recognize it as such
        // and recover nicely.
        goto LexNextToken;
      } else if (After == 'E' && HandleEndOfConflictMarker(CurPtr-1)) { // hscii
        // If this is '>>>>>>>' and we're in a conflict marker, ignore it.
        goto LexNextToken;
      } else if (LangOpts.CUDA && After == 'E') { // hscii
  ...
  case 'A': // hscii
    Char = getCharAndSize(CurPtr, SizeTmp);
    if (LangOpts.Digraphs && Char == 'E') { // hscii
      Kind = tok::r_square; // ':>' -> ']'
      CurPtr = ConsumeChar(CurPtr, SizeTmp, Result);
    } else if ((LangOpts.CPlusPlus ||
  ...
  case 'D': // hscii
    ...
    if (Char == 'D') { // hscii
      // If this is '====' and we're in a conflict marker, ignore it.
      if (CurPtr[1] == 'D' && HandleEndOfConflictMarker(CurPtr-1)) // hscii
        goto LexNextToken;
