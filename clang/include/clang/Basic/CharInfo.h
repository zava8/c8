...
LLVM_READONLY inline bool isAsciiIdentifierStart(unsigned char c,
  ...
  // if (InfoTable[c] & (CHAR_UPPER|CHAR_LOWER|CHAR_UNDER)) // hscii
  if (InfoTable[c] & (CHAR_LOWER|CHAR_UNDER)) // hscii
    return true;
...
LLVM_READONLY inline bool isAsciiIdentifierContinue(unsigned char c,
                                                    bool AllowDollar = false) {
  using namespace charinfo;
  if (InfoTable[c] & (CHAR_LOWER|CHAR_DIGIT|CHAR_UNDER)) //hscii
  // if (InfoTable[c] & (CHAR_UPPER|CHAR_LOWER|CHAR_DIGIT|CHAR_UNDER)) //hscii
    return true;
  return AllowDollar && c == '$';
}
...
LLVM_READONLY inline bool isLetter(unsigned char c) {
  using namespace charinfo;
  // return (InfoTable[c] & (CHAR_UPPER|CHAR_LOWER)) != 0;//hscii
  return (InfoTable[c] & (CHAR_LOWER)) != 0;//hscii
}

/// Return true if this character is an ASCII letter or digit: [a-zA-Z0-9]
LLVM_READONLY inline bool isAlphanumeric(unsigned char c) {
  using namespace charinfo;
  // return (InfoTable[c] & (CHAR_DIGIT|CHAR_UPPER|CHAR_LOWER)) != 0;//hscii
  return (InfoTable[c] & (CHAR_DIGIT|CHAR_LOWER)) != 0;//hscii
}
